@echo off
REM Generate keypairs for server for both RSA and DSA (default)
...
REM Generate keypairs for client for both RSA and DSA (default)
...
REM Export the server certificates
...
REM Export the client certificates
...
REM Import both server and client certificates to truststore
keytool -import -alias servercert -file server.cert -keystore truststore -storepass 123456 -noprompt
keytool -import -alias serverrsacert -file serverrsa.cert -keystore truststore -storepass 123456 -noprompt
keytool -import -alias clientcert -file client.cert -keystore truststore -storepass 123456 -noprompt
keytool -import -alias clientrsacert -file clientrsa.cert -keystore truststore -storepass 123456 -noprompt
